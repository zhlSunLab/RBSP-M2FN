...
def train(model, train_dataset, test19, save=None, batch_size=32, train_number=6, epochs=30, train_files=None,
          test19_files=None):
    ...
    ## load dataset
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, sampler=train_samples,
                                               pin_memory=(torch.cuda.is_available()), num_workers=6, drop_last=False)
    valid_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, sampler=valid_samples,
                                               pin_memory=(torch.cuda.is_available()), num_workers=6, drop_last=False)
    ...
    for epoch in range(epochs):
        ...
        print(f'TRAIN------Epoch:{epoch}, loss:{t_loss:0.4f}')
        print(f'VALID------Epoch:{epoch}, loss:{v_loss:0.4f}, precision:{p_max:0.4f}, recall:{r_max:0.4f}, auc:{auc:0.4f}, mcc:{mcc:0.4f} ')
        ### for the test set
        test19_loss, p_max19, r_max19, auc19, t_max19, mcc19, predictions19, v_labels19, v_predicts19, nucle_nums19, rna_names19 = test_epoch(
            model=model_wrapper, load=test19_loader, is_test=test19_loader)
        print('TEST19------Epoch:%02d,Precision19:%0.4f,Recall19:%0.4f,AUC19:%0.4f,T_max19:%0.4f,MCC19:%0.4f\n' % (
            (epoch + 1), p_max19, r_max19, auc19, t_max19, mcc19))

        if auc > best_auc:
            best_auc = auc
            threadhold = t_max
            torch.save(model.state_dict(), os.path.join(save, './modelr-11.dat'))
...
if __name__ == '__main__':
    ...
    for numbers in range(1, 60):
        ...
        train(model, train_data, test19_data, path_dir, batch_size, numbers, epochs, train_index_files,
              test19_index_files)
        print("Training_(%s/60) finished!" % (numbers + 1))
